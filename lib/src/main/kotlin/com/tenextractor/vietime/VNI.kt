...
object VNI {
    ...
    fun VNIToVietnamese(input: String): String {
        ...
        for ((index, ch) in output.withIndex()) {
            if (Maps.VOWELS.contains(ch) && !(lowercaseVowel.length > 1 && index == 1 && setOf("qu", "gi").contains(output.substring(0..<2).lowercase()))) {
                firstVowelIndex = index
                break
            }
        }

        val toneMarkPosition = Common.getToneMarkPosition(output, firstVowelIndex, lowercaseVowel.length)
